...
using dnlib.PE;
using ISTAlter.de4dot.blocks;
using ISTAlter.de4dot.blocks.cflow;
using ISTAlter.de4dot.code.AssemblyClient;
using ISTAlter.de4dot.code.AssemblyData;
using ISTAlter.de4dot.code.deobfuscators;
using ISTAlter.de4dot.code.renamer;
	...
	public class ObfuscatedFile : IObfuscatedFile, IDeobfuscatedFile {
		...
		void ReloadModule(byte[] newModuleData, DumpedMethods dumpedMethods) {
			...
			using (var oldModule = module) {
				module = assemblyModule.Reload(newModuleData, CreateDumpedMethodsRestorer(dumpedMethods), deob as dnlib.DotNet.IStringDecrypter);
